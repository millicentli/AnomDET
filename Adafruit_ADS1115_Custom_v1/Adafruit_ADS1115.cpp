...
int16_t Adafruit_ADS1115::getLastConversionResults()
{
  ...
  // Read the conversion results
  uint16_t res = readRegister(m_i2cAddress, ADS1115_REG_POINTER_CONVERT) >> m_bitShift; //shifts so bits can use a different address - can use a different hex number/address but will interfere with the current bits
  if (m_bitShift == 0)
  {
    return (int16_t)res;
  }
